...
async function seed() {
  ...
  for (const acc of accounts) {
    ...
    if (!user) {
      ...
      const wallet = walletsRepo.create({
        userId: user.id,
        balance: acc.balance,
        currency: 'W',
      });
  ...
  // Demo live streams
  const niko = await usersRepo.findOne({ where: { username: 'you' } });
  const luna = await usersRepo.findOne({ where: { username: 'alex' } });
  ...
  if (liveCount === 0 && rita) {
    await streamsRepo.save([
      streamsRepo.create({
        ...
        donationsTotal: 3200,
        startedAt: new Date(),
        livekitRoom: 'demo_room_1',
      }),
      streamsRepo.create({
        title: 'AI Battle Night ⚔️',
        creatorId: niko?.id ?? rita.id,
        status: StreamStatus.LIVE,
        category: 'Gaming',
        viewerCount: 1800,
        likesCount: 8900,
  ...
  if (premiumCount === 0 && rita) {
    await premiumRepo.save([
      premiumRepo.create({
        ...
        content: 'Exclusive studio session footage and unreleased tracks.',
        price: 50,
        isPremium: true,
      }),
      premiumRepo.create({
        creatorId: rita.id,
        title: 'Free Preview',
        content: 'Welcome to my premium channel!',
        price: 0,
        isPremium: false,
      }),
    ]);
  ...
  console.log('  creator@wplus.dev / password123  (Rita, creator)');
  console.log('  user@wplus.dev / password123     (You, creator)');
  console.log('  viewer@wplus.dev / password123   (Alex, viewer)');
  console.log('  admin@wplus.dev / admin123       (Admin)');
